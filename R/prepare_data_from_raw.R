...
ibq_clean_data <- function(results, min_number_of_quantified_peps, ... ){
    ...
    protein = clean[["protein"]] %>%
      map(.,
          . %>% mutate(gene_name = protein_id) %>%
            select(., gene_name, contains("signal_sum")) %>% select(-contains("signal_sum_131H")) ),
...
ibq_clean_data_keep_mods <- function(results, min_number_of_quantified_peps, ... ){
